          ...
          <React.Fragment key={index}>
            <div className="group vt-item max-w-[800px] ml-auto mr-auto xl:ml-0 xl:even:mr-0 xl:w-[50%] xl:-mt-[8rem] xl:pr-10 xl:even:pr-0 xl:even:pl-10 xl:first:mt-0 xl:even:ml-auto relative ">
              ...
              >             
                ...
                <h3 className="font-semibold text-2xl capitalize text-center md:text-left">{item.company}</h3>
                <p className="font-normal text-lg text-gray-500 mb-8 text-center md:text-left">{item.location}</p>  
                  ...
                  item.jobs.map((job, index2) => (                  
                    <React.Fragment key={index2}>
                      <li className="flex flex-col mb-10 last:mb-0 pl-4 md-pl-8 border-l-8 border-l-slate-500/10 ">
                        <h4 className="text-slate-200/95 text-lg font-semibold capitalize mb-1">{job.title}</h4>
                        <p className="font-normal text-gray-500 mb-5">{job.date}</p>  
                        <p className="font-normal text-gray-300 block mb-7">{job.short}</p> 
                        <ul className="mb-3">
                          <p className="mb-2 text-sm text-gray-500">Featured Skills</p>
                          {
                            job.skills.map((skill, index3) => (   
                              <React.Fragment key={index3}>                     
                                <li  className="text-gray-300 inline-block text bg-zinc-700/40 px-2 py-1 mr-2 mb-2 rounded-full">{skill}</li> 
                              </React.Fragment>
                            ))
                          }
                        </ul>            
